...
from mailLog.views import tablemaillog
...
def loginserver(request):
    #try: 
            ...
            if request.method=='POST':
                ...
                src = "D:\Logs\mail1.log.1"
                dst = "D:\4A\2eme_semestre\projectDjango\Logs"
                try :                    
...
def setsyslog(request):
    file = open('D:\Logs\mail1.log.1', "r")
...
def setmaillog(request):
    file = open('D:\Logs\mail.log.1', "r")
    lines = file. readlines()
    file.close()
    logs= user.servers.get(host=request.session['server']).syslogs
    testlast=1
